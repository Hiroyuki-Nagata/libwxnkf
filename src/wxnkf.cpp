...
std::string wxNKF::WxToMultiByte(const wxString inputString, const wxString option) {
     ...
     // convert wxString to UTF-8 std::string
     const char* inputStrChar = inputString.ToUTF8();
     std::string iConv(inputStrChar);
     std::string oConv;
     
     // convert UTF-8 string to other multibyte string
     if (0 != KanjiConvert(iConv, oConv))
	  return "";
 ...
 */
int wxNKF::KanjiConvert(std::string iConv, std::string oConv) {

     // get iterator of input string
     std::string::iterator it = iConv.begin();
     nkf_char c1 = 0, c2 = 0, c3 = 0, c4 = 0;
     int shift_mode = 0; /* 0, 1, 2, 3 */
     int g2 = 0;
     int is_8bit = FALSE;
     ...
     /**
      * start character convert process
      */
     while (it != iConv.end()) {
	  /* set first char */
	  c1 = *it;
	  ...
	  if (!wxEnc->inputMode)
 	  ...
 	  if (c2) {
 	       ...
 	       if (c2 > DEL) {
 		    ...
 		    if (!nkfFlags[estab_f] && !nkfFlags[mime_decode_mode]) {
 			 /** 
 			  * character code guess convert mode (omitted)   
 			  */
 		    } else {
 			 /* in case of already established */
 			 if (c1 < 0x40) {
 	       ...
 	       wxEnc->Oconv(0, c1, nkfFlags, oConv);
 	       NEXT;
 	  } else {
 	       /* first byte */
 	       if (wxEnc->inputMode == JIS_X_0208 && DEL <= c1 && c1 < 0x92) {
 		    ...
 		    if (*it++ == EOF) {
 			 wxEnc->Oconv(0, ESC, nkfFlags, oConv);
 			 LAST;
 		    } else if (c1 == '&') {
 			 /* IRR */
 			 if ((c1 = *it++) == EOF) {
 				...
 				NUM : 2 0 3 4 5 X 1
 			      */
 			      ...
 			      if ((c1 = *it++) == EOF)
 				   LAST;
 			      while (SP <= c1 && c1 <= 'z') {
 				   wxEnc->Oconv(0, c1 + c3, nkfFlags, oConv);
 				   if ((c1 = *it++) == EOF)
 					LAST;
 			      }
 			      SKIP;
 	  ...
 	  case EUC_JP:
 	  case UTF_8:
 	       switch (wxEnc->Iconv(c2, c1, 0, nkfFlags, oConv)) { /* can be EUC / SJIS / UTF-8 */
 	       case -2:
 		    /* 4 bytes UTF-8 */
 		    if ((c3 = *it++) != EOF) {
 			 c3 <<= 8;
 			 if ((c4 = *it++) != EOF) {
 			      wxEnc->Iconv(c2, c1, c3 | c4, nkfFlags, oConv);
 			 }
 		    }
 		    break;
 	       case -1:
 		    /* 3 bytes EUC or UTF-8 */
 		    if ((c3 = *it++) != EOF) {
 			 wxEnc->Iconv(c2, c1, c3, nkfFlags, oConv);
 		    }
 		    break;
 	       }
 	       break;
 	  ...
 	  case JIS_X_0213_1:
 	       ...
 	       wxEnc->Oconv(c2, c1, nkfFlags, oConv); /* this is JIS, not SJIS/EUC case */
 	       break;
 	  case JIS_X_0212:
 	       wxEnc->Oconv(PREFIX_EUCG3 | c2, c1, nkfFlags, oConv);
 	       break;
 	  case JIS_X_0213_2:
 	       wxEnc->Oconv(PREFIX_EUCG3 | c2, c1, nkfFlags, oConv);
 	       break;
 	  default:
 	       wxEnc->Oconv(wxEnc->inputMode, c1, nkfFlags, oConv); /* other special case */
 	  }
